...
double CPPLogLikelihoodEval(arma::mat Y,
  ...
  DiffMat1.each_row() /= SigmaVec.t();
  
  Term1 = -n * accu(log(SigmaVec));
  Term2 = -0.5 * accu(square(DiffMat1));
...
double CPPCriterionFunction(int kInd,
  ...
  LogLikDataSet = (-n*p/2.0) - (n * accu(log(sigsq_hat_diag)) / 2.0); //scaled by n*p
  ...
  CriterionStor = (-2 * LogLikDataSet) + (kInd * maxint * log(minint)); //logL already scaled by n*p
...
Rcpp::List CPPFABLEPostMean(arma::mat Y, 
  ...
  return Rcpp::List::create(Rcpp::Named("CovPostMean") = CovPostMean,
                            Rcpp::Named("kEst") = k);
...
// [[Rcpp::export]]
Rcpp::List CCFABLEPostProcessingSubmatrix_Optimized(Rcpp::List FABLEOutput,
                                                    double alpha,
                                                    arma::uvec SelectedIndices) {
  ...
  // Pre-extract Lambda blocks for selected indices
  std::vector<arma::mat> LambdaBlocks(SubLength);
  for(int idx = 0; idx < SubLength; ++idx) {
    int col_start = SelectedIndices(idx) * k;
    int col_end = col_start + k - 1;
    LambdaBlocks[idx] = LambdaSamples.cols(col_start, col_end);
  }
  
  for(int j1 = 0; j1 < SubLength; ++j1) {
    int FirstIndex = SelectedIndices(j1);
    arma::mat Lambdaj1Samples = LambdaBlocks[j1];
    ...
    for(int j2 = j1; j2 < SubLength; ++j2) {
      int SecondIndex = SelectedIndices(j2);
      arma::mat Lambdaj2Samples = LambdaBlocks[j2];
      
      // Compute elementwise products and sum over rank
      arma::vec Lj1j2Samples = sum(Lambdaj1Samples % Lambdaj2Samples, 1);
